...
import (
	...
	"crashsaver/websocket/internal/services"
	"log"
	"time"
...
func (c *Client) readMessages(m *Manager) {
	...
	for {
		...
		case "create", "update", "delete", "get":
			services := services.NewIncidentService(c.manager.Incidents, c.manager)
			services.HandleWebSocketMessage(payload)
			// c.manager.MessageToAllClients()
		default:
